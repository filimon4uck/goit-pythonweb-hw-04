...
def get_random_filename():
    random_value = '-0123456789=@ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnop' \
                   'qrstuvwxyz'
...
def generate_text_files(path):
    ...
    with open(
        path / f"{get_random_filename()}.{choice(documents).lower()}",
        "wb"
    ) as f:
...
def generate_archive_files(path):
    ...
    shutil.make_archive(
        f"{path}/{get_random_filename()}",
        f'{choice(archive).lower()}',
        path
    )
...
def generate_folders(path):
    folder_name = ['temp', 'folder', 'dir', 'tmp', 'OMG', 'no_way', 'find_it']
    folder_path = Path(
        f"{path}/" + '/'.join(choices(
            folder_name,
            weights=[10, 10, 1, 1, 1, 1, 1],
